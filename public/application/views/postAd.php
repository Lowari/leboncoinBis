        ...
        <fieldset>
            ...
            <div class="form-group">
                ...
                <select class="form-control" id="category" name="category">
                  ...
                  <?php foreach($cat as $category){ if(set_value("category") == $category->id){?><option value="<?=$category->id?>" selected><?=$category->name?></option> <?php } ?>
                    <option value="<?=$category->id?>"><?=$category->name?></option>
                <?php  }  ?>
                </select>
                <?php if(set_value("category") == "0"){echo '<small class="form-text text-danger">Veuilez selectionner une catégorie</small>';}; ?>
            </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control" id="region" name="region">
                    ...
                    <?php foreach($region as $reg){  if(set_value("region")){if(set_value("region") == $reg->number){?><option value="<?=$reg->number;?>" selected><?php echo $reg->number." - ".$reg->name;?></option> <?php } elseif($zip == $reg->number){?> <option value="<?=$reg->number?>" selected><?php echo $reg->number." - ".$reg->name;?></option>  <?php }; }?>
                    <option value="<?=$reg->number?>"><?php echo $reg->number." - ".$reg->name;?></option>
                <?php  }  ?>
                </select>
                <?php if(set_value("region") == "0"){echo '<small class="form-text text-danger">Veuilez selectionner un departement</small>';}; ?>
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="phone" name="phone" value="<?php if(set_value('phone')){ echo set_value("phone"); }else{ echo $user[0]->phone; } ;?>" placeholder="Entrer votre téléphone">
                <?=form_error("phone");?>
            </div>
            <div class="form-group">
                <label for="mail">Mail de contact</label>
                <input type="mail" class="form-control" id="mail" name="mail" value="<?php if(set_value('mail')){ echo set_value("mail"); }else{ echo $user[0]->mail; } ;?>" placeholder="Entrer votre mail">
                <?=form_error("mail");?>
            </div>
